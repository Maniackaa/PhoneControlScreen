...
amount_ok = (50, 610)
card_field = Target(img=Path('media/total/m10card.bmp'), x=30, y=130)


exp_from_card = (10, 80)
cvv_from_card = (110, 80)
...
def insert_card_data():
    try:
        ...
        while True:
            ...
            target_point = find_target_in_image(make_screenshot(), card_field, thresh1)
            if target_point:
                print('Нашел карту')
                # pg.click(target_point[0], target_point[1])
                break
            else:
                print(f'Не нашел карту. Жду еще {after_amount_limit - count} сек.')
